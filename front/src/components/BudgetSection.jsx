            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {budgets.length > 0 ? (
                    budgets.map((b) => {
                        ...
                        return (
                            <div key={b.category_id} className={`bg-[#101627] p-6 rounded-xl border transition-all ${isOverLimit ? 'border-red-500/50' : 'border-[#283046] hover:border-[#3b4461]'}`}>
                                <div className="flex justify-between items-center mb-4 text-sm font-medium">
                                    <span className="text-slate-300">{b.category_name}</span>
                                    {/* PAKEISTAS SPALVOS STILIUS TEKSTUI */}
                                    <span className={isOverLimit ? "text-red-500" : "text-[#7367f0]"}>
                                        €{used.toFixed(0)} <span className="text-slate-500 font-normal">/ €{limit.toFixed(0)}</span>
                                    </span>
                                </div>
                                ...
                                <div className="flex justify-between items-center">
                                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isOverLimit ? 'text-red-500' : 'text-slate-500'}`}>
