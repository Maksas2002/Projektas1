    ...
    <div className="min-h-screen bg-[#020b33] text-white flex flex-col">
      <header className="border-b border-white/10">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav>
            <Link to="/" className="text-sm text-white hover:text-blue-300">Home</Link>
          </nav>
          <Link
            to="/login"
      ...
      <main className="flex flex-1 items-center justify-center px-6 py-14">
        <section className="w-full max-w-md">
          <div>
            <h1 className="text-center text-[2.5rem] font-semibold text-white">Welcome back</h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center gap-4 pt-10">
            <div>
              ...
              />
              {errors.email && <span className="mt-2 block text-sm text-red-500">{errors.email.message}</span>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="block h-12 w-full rounded-[10px] border border-white/10 bg-[#1f2747] px-3 text-gray-300 outline-none"
              />
              {errors.password && <span className="mt-2 block text-sm text-red-500">{errors.password.message}</span>}
            </div>

            <button type="submit" className="mt-2 cursor-pointer rounded-[10px] bg-blue-500 p-3 text-white transition hover:bg-blue-400 font-medium">
              Sign In
            </button>

            {error && <p className="mt-4 p-3 bg-red-500/10 border border-red-500/50 rounded-lg text-center text-red-500 text-sm">{error}</p>}
          </form>
        </section>
      </main>
    </div>
